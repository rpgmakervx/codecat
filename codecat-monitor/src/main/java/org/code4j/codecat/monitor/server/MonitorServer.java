package org.code4j.codecat.monitor.server;/**
...
import org.code4j.codecat.monitor.handler.MonitorServerChildHandler;
...
public class MonitorServer {
    ...
    private void launch(int port){
        logger.debug("monitor server is starting...");
        ...
        try {
            ...
            f = b.bind(port).sync();
            logger.info("monitor server has started");
            ShellInvoker.execute(ShellInvoker.STARTUP_SERVER
                    ,String.valueOf(PortCounter.getPort()));
            logger.info("real server is drived by monitor server");
